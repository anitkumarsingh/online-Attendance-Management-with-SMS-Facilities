		   ...
		   try {
			   ...
			   File itemName =new File(item.getName());
                           ...
                           if(fileSize>20*1024*1024){
                               %>
                               <script>
                                   alert("File is too big: <%=fileSize%>");
                               </script>
                               <%
                               return;
                           }
                               ...
                               if(value[1].trim().equalsIgnoreCase("staff")){
                                   if(value[0].trim().equalsIgnoreCase("all")){
                                       String sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                               "select '"+session.getAttribute("userid")+"','"+subject+"',staff_id,'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"' from staff where user_type not like 'Admin'";
                                       st.executeUpdate(sql);
                                    }
                                   else{
                                       
                                       String sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                                "values ('"+session.getAttribute("userid")+"','"+subject+"',(select staff_id from staff where user_name like '"+value[0].trim()+"'),'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"')";
                                   ...
                                   if(value[0].trim().equalsIgnoreCase("all")){
                                        String sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                               "select '"+session.getAttribute("userid")+"','"+subject+"',student_id,'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"' from students";
                                       st.executeUpdate(sql);
                                    }
                                   else{
                                       String sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                               " values('"+session.getAttribute("userid")+"','"+subject+"','"+value[0]+"','"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"')";
                                       st.executeUpdate(sql);
                                       }
                               }
                                   ...
                                   if(semester.equals(""))
                                       sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                           "select '"+session.getAttribute("userid")+"','"+subject+"',student_id,'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"'  from elective_students where subject_id like '"+sub_sec[0]+"'";
                                   else
                                       sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                           "select '"+session.getAttribute("userid")+"','"+subject+"',student_id,'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"' from students where semester="+semester +" and section ='"+sub_sec[1]+"'";
                                   out.print(sql);
                                   st.executeUpdate(sql);
                               }
                               else if(value[1].trim().equalsIgnoreCase("semester")){
                                   String sql="insert into mail (sender_id,`subject`,`to`,msg,`timestamp`,attachment) " +
                                               "select '"+session.getAttribute("userid")+"','"+subject+"',student_id,'"+body+"',now(),'"+savedFile.getName().replace("'", "''").replace("\\", "\\\\")+"' from students where semester ="+value[0];
                                   st.executeUpdate(sql);
                               }
